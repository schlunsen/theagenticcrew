...
#figure(
  image("../../assets/illustrations/crew/ch12-do-it-yourself.jpg", width: 80%),
  caption: [_De vegades l'eina correcta són les teves pròpies mans._],
)

L'habilitat més important per treballar amb agents no és saber com utilitzar-los. És saber quan _no_ fer-ho.
...
Hi ha tasques on la velocitat de l'agent és el que importa i tasques on en realitat és el problema. La distinció normalment es redueix a una paraula: _criteri_.

Un agent pot redactar un correu. Però hauria de redactar el correu que diu a un client de tota la vida que puges els preus? Probablement no — almenys no sense una edició a fons. El to, la relació, l'equilibri delicat entre honestedat i diplomàcia — això és territori humà.
...
Un agent pot escriure un informe. Però hauria d'escriure l'avaluació de rendiment d'algú del teu equip? Les paraules potser són correctes. El _pes_ d'aquelles paraules — que venen d'una eina en lloc d'una persona — ho canvia tot.

El patró: els agents són excel·lents _produint_ coses. No són fiables _decidint_ coses que afecten les persones. Utilitza'ls per al primer. Reserva't el segon.
...
Un agent treballa amb el que li dones. Si la informació més important és del tipus que no pots articular fàcilment — sentiments, instints, relacions — l'agent se la perdrà. I no sabrà que se l'està perdent.
...
De vegades el valor d'una tasca _és_ el temps que porta. Escriure una nota d'agraïment sincera. Pensar a fons una decisió estratègica. Esbossar una idea al paper per veure si queda bé. Llegir un document lentament per entendre'n els matisos.
...
+ *Puc verificar el resultat?* Si la resposta és sí, probablement l'agent és segur de fer servir. Si no pots saber si el resultat és correcte — perquè et falta l'experiència, o la verificació portaria tant de temps com fer-ho tu mateix — pensa-t'ho dues vegades.

+ *Què passa si és incorrecte?* Si un error és fàcil de detectar i barat de corregir, deixa que l'agent treballi. Si un error és difícil de detectar i car de recuperar, mantingues un humà en el procés — o fes-ho tu mateix.

+ *L'element humà és el que importa?* Si el valor de la tasca ve del fet que la facis _tu_ — la relació, el criteri, l'expressió creativa — llavors fer-ho més ràpid amb un agent en realitat destrueix valor.
