...
*Maintain agent instruction files.* A project-level context file (like `CLAUDE.md`) that describes architecture, conventions, and current priorities persists across sessions and gets read automatically. We cover what makes a good one — and how to structure them for maximum impact — in the Conventions chapter.
...
Context windows are ephemeral. When a session ends, everything the agent learned vanishes. For short tasks, this doesn't matter. But real engineering work spans days, sometimes weeks. If you don't plan for session boundaries, you'll waste enormous time re-establishing context the agent already had.
...
*Write session summaries.* When you finish a complex session, have the agent summarise what it accomplished, what's left, and what it learned. Save it somewhere the next session will find it — a comment on the ticket, a note in the repo. You've preserved hours of accumulated understanding in a few paragraphs.

But these are manual, discipline-dependent solutions. The deeper answer — active memory systems that capture and retrieve knowledge automatically across sessions and across teams — is the subject of the Ship's Log chapter.

== Context as Architecture

As you get better at agentic engineering, you start designing your systems _for_ context. File naming, directory structure, and conventions all serve double duty — they help humans _and_ they help agents navigate. The Conventions chapter covers these structural choices in depth. Here, let's focus on the context dimensions that are unique to code architecture.
