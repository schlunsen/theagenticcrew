...
A growing convention in agentic engineering is the agent instruction file — a document at the root of your project that tells the agent what it needs to know. Not a README for humans. A briefing for agents. The most common format is `CLAUDE.md` (used by Claude Code), but the pattern is tool-agnostic: Cursor uses `.cursorrules`, Windsurf uses `.windsurfrules`, GitHub Copilot reads `.github/copilot-instructions.md`. The name varies; the principle is identical.
...
Your project structure persists across sessions. Your file naming persists. Your `CLAUDE.md` persists. Your linter rules persist. Everything you encode into the shape of your project is there every time the agent opens its eyes. Conventions aren't just about consistency for humans — they're _external memory_ for agents. Every convention you establish is a lesson the agent doesn't have to relearn.
...
Let's be honest about the cost. Writing a `CLAUDE.md` takes an afternoon. Setting up linters and formatters takes a day. Refactoring an inconsistent codebase to follow a single pattern takes a week. It feels like overhead.
