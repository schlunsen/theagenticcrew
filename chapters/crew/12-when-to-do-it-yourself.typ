...
#figure(
  ...
  caption: [_Sometimes the right tool is your own hands._],
)

The most important skill in working with agents isn't knowing how to use them. It's knowing when _not_ to.
...
There are tasks where the agent's speed is the point and tasks where it's actually the problem. The distinction usually comes down to one word: _judgment_.
...
An agent can write a report. But should it write the performance review for someone on your team? The words might be fine. The _weight_ of those words — coming from a tool rather than a person — changes everything.

The pattern: agents are excellent at _producing_ things. They're unreliable at _deciding_ things that affect people. Use them for the former. Keep the latter for yourself.
...
Sometimes the value of a task _is_ the time it takes. Writing a heartfelt thank-you note. Thinking through a strategic decision. Sketching an idea on paper to see if it feels right. Reading a document slowly to understand the nuance.
...
+ *Can I verify the output?* If yes, the agent is probably safe to use. If you can't tell whether the output is correct — because you lack the expertise, or the verification would take as long as doing it yourself — think twice.

+ *What happens if it's wrong?* If a mistake is easy to catch and cheap to fix, let the agent run. If a mistake is hard to detect and expensive to recover from, keep a human in the loop — or do it yourself.

+ *Is the human element the point?* If the value of the task comes from _you_ doing it — the relationship, the judgment, the creative expression — then doing it faster with an agent actually destroys value.
